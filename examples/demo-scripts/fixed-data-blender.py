#!/usr/bin/env python3
"""
Fixed Plex Data Blender - Corrected database extraction
"""
...
class FixedPlexDataBlender:
    ...
    def extract_from_database(self, db_path: str, server_name: str) -> List[Dict[str, Any]]:
        """Extract watch history from a Plex database file with corrected queries"""
        try:
            ...
            # Corrected query using GUID to join tables
            query = """
            SELECT 
                m.title,
                m.year,
            ...
            LEFT JOIN metadata_item_views v ON m.guid = v.guid
            ...
            try:
                ...
                if rows:
                    ...
                    print(f"⚠️ No watched movies found in {server_name} database")
                    return []
                        
            except sqlite3.OperationalError as e:
                print(f"⚠️ Database query error for {server_name}: {e}")
                conn.close()
                return []
    ...
    def load_backup_data(self, backup_directory: str = "backups") -> List[Dict[str, Any]]:
        ...
        if os.path.exists(azog_path):
            ...
            # Try the main database file first
            main_db = f"{azog_path}/com.plexapp.plugins.library.db"
            if os.path.exists(main_db):
                data = self.extract_from_database(main_db, "azog")
        ...
        if os.path.exists(bilbo_path):
            ...
            # Try the main database file first
            main_db = f"{bilbo_path}/com.plexapp.plugins.library.db"
            if os.path.exists(main_db):
                data = self.extract_from_database(main_db, "bilbo")
                all_backup_data.extend(data)
    ...
    def calculate_statistics(self, movies: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for movie in movies:
            ...
            if rating and rating != "0" and rating != "None" and rating != "":
                try:
